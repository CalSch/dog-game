...
@onready var PLAYER_SPRITE: AnimatedSprite2D = get_node(PLAYER_SPRITE_PATH) if PLAYER_SPRITE_PATH else $Sprite2D ## The [Sprite2D] of the player character

## The path to the character's [CharacterBody2D] node.  If no node path is provided the [param PLAYER_SPRITE] will be set to [param $Sprite2D] if it exists.
@export_node_path("CharacterBody2D") var PLAYER_NODE_PATH: NodePath
@onready var PLAYER_BODY: CharacterBody2D = get_node(PLAYER_NODE_PATH) if PLAYER_SPRITE_PATH else $".." ## The [Sprite2D] of the player character
...
@onready var mpp: MPPlayer = get_parent().get_parent()
...
func manage_state() -> void:
	if PLAYER_BODY.velocity.y == 0:
		...
		if can_wall_jump and PLAYER_BODY.is_on_wall_only() and get_input_direction().x != 0:
			state = WALL_SLIDE
		else:
			state = FALL
...
func get_inputs() -> Dictionary:
	return {
		...
		jump_strength = Input.get_action_strength(mpp.ma(ACTION_JUMP)),
		jump_pressed = Input.is_action_just_pressed(mpp.ma(ACTION_JUMP)),
		jump_released = Input.is_action_just_released(mpp.ma(ACTION_JUMP)),
		sprint_strength = Input.get_action_strength(mpp.ma(ACTION_SPRINT)) if ENABLE_SPRINT else 0.0,
	}


## Gets the X/Y axis movement direction using the input mappings assigned to the ACTION UP/DOWN/LEFT/RIGHT variables
func get_input_direction() -> Vector2:
	var x_dir: float = Input.get_action_strength(mpp.ma(ACTION_RIGHT)) - Input.get_action_strength(mpp.ma(ACTION_LEFT))
	var y_dir: float = Input.get_action_strength(mpp.ma(ACTION_DOWN)) - Input.get_action_strength(mpp.ma(ACTION_UP))
...
func handle_gravity(delta: float) -> void:
	...
	if can_wall_jump and state == WALL_SLIDE and not jumping:
		PLAYER_BODY.velocity.y = clampf(PLAYER_BODY.velocity.y, 0.0, WALL_SLIDE_SPEED)
	
	if not PLAYER_BODY.is_on_floor() and can_jump:
		coyote_time()
...
func apply_velocity(delta: float, move_direction: Vector2) -> void:
	...
	PLAYER_BODY.velocity.x += move_direction.x * ACCELERATION * delta * (sprint_strength if PLAYER_BODY.is_on_floor() else 1.0)
	PLAYER_BODY.velocity.x = clamp(PLAYER_BODY.velocity.x, -MAX_SPEED * abs(move_direction.x) * sprint_strength, MAX_SPEED * abs(move_direction.x) * sprint_strength)


## Applies friction to the horizontal axis when not moving using the [param FRICTION] and [param AIR_RESISTENCE] values
func apply_friction(delta: float) -> void:
	var fric: float = FRICTION * delta * sign(PLAYER_BODY.velocity.x) * -1 if PLAYER_BODY.is_on_floor() else AIR_RESISTENCE * delta * sign(PLAYER_BODY.velocity.x) * -1
	if abs(PLAYER_BODY.velocity.x) <= abs(fric):
		PLAYER_BODY.velocity.x = 0
	else:
		PLAYER_BODY.velocity.x += fric
...
func handle_jump(delta: float, move_direction: Vector2, jump_strength: float = 0.0, jump_pressed: bool = false, _jump_released: bool = false) -> void:
	if (jump_pressed or should_jump) and can_jump:
		...
		buffer_jump()
	elif jump_strength == 0 and PLAYER_BODY.velocity.y < 0:
		cancel_jump(delta)
	elif can_wall_jump and not PLAYER_BODY.is_on_floor() and PLAYER_BODY.is_on_wall_only():
		can_jump = true
		wall_jump = true
		jumping = false
...
func cancel_jump(delta: float) -> void:
	jumping = false
	PLAYER_BODY.velocity.y -= JUMP_CANCEL_FORCE * sign(PLAYER_BODY.velocity.y) * delta
